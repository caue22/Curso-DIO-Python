...
from store.main import app  # importa seu FastAPI app
...
@pytest.mark.asyncio
async def test_controller_post_should_return_insert_error(client, products_url):
    """Simula erro de inserção e espera HTTP 500"""

    class FakeUsecase(ProductUsecase):
        async def create(self, body: ProductIn):
            raise Exception("DB error")

    # Override para o teste
    app.dependency_overrides[ProductUsecase] = lambda: FakeUsecase()
...
async def test_patch_should_return_not_found(client, products_url):
    ...
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {
        "detail": "Product not found with id: 4fd7cd35-a3a0-4c1f-a78d-d24aa81e7dca"
    }
